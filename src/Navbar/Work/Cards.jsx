...
const Cards = () => {
    ...
    {/* isse sm: breakpoiny pe thikk krne ka h */}
        <div classNameName="mx-2 md:mx-[4rem] my-4 overflow-hidden relative bg-transparent px-2 md:px-[1rem] py-2 md:py-[1rem] md:my-[3rem] ">
            <ProgrammingHead head = {"OUR WORKING PROGRAMMING LANGUAGES"} />
            <div className="md:grid mx-[5rem] md:grid-cols-3 md:justify-evenly md:gap-[2rem] space-y-2  md:mx-[2rem] md:mb-[2rem] md:mt-[4rem]">
                {
                    CardsApi.map((data, i) => {
                        return <>
                            <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-sky-200 dark:border-gray-700 hover:scale-105 duration-300 delay-150 ease-in-out transition-all hover:shadow-lg shadow-white hover:drop-shadow-xl">
...
const ProgrammingHead = ({head}) => {
    // document.getElementById("programhrad").classList.add(color)
    return <>
        <div  className="text-center px-auto overflow-hidden relative mt-[1rem] md:my-[1rem] mb-4">
            <p id="programhrad" className="text-xl selection:bg-[#111827]  font-mono  font-bold tracking-[0.5rem] underline underline-offset-8 md:text-[1.5rem] md:underline-offset-[1rem]">{head}</p>
        </div>
    </>
    
}

export {ProgrammingHead, Cards}